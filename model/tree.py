...
#
def head_to_treeEval(head,deprel,relpairs,build_mid):
# # def head_to_tree(head,deprel, len_):
    """
    Convert a sequence of head indexes into a tree object.
    """
    ...
    ancester, sdp_path, minsdplength, pair_id=getSDPAncestorEval(root,pairlist)
    subj_nodes=pairlist[pair_id][0]
    obj_nodes=pairlist[pair_id][-1]
    ...
    #                     nodestack.append(h)
    #                 cur=h
    return root,domains+1,minsdplength,relpairs[pair_id],midhead
...
def tree_to_dis(cur,src,sdpdis,domain):
    ...
    if (len(cur.children)!=0):
        for child in cur.children:
            if cur.sdpdis==0 and child.domainid==-1:
                # domain = min(25, domain + 1)
                domain=tree_to_dis(child, cur, sdpdis, domain)
            else:
                domain = tree_to_dis(child, cur, sdpdis + 1, domain)
                # if endnode is None:
                #     endnode=cendnode
    ...
    if (cur.parent is not None) and (cur.parent!=src):
        if cur.sdpdis==0 and cur.parent.domainid==-1:
            # domain=min(25,domain+1)
...
def tree_to_adj(sent_len,domains, tree,directed=False):
    """
    ...
    return depmap,ret,rel,rel,domain,domain_id,redomain_id